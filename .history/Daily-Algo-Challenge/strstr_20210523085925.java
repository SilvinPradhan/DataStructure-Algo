...
public class strstr {
    ...
    public static int strCheck(String haystack, String word){
        if(haystack.length()<word.length() || haystack == null || word == null || haystack.length() == 0 || word.length()==0){
            return -1;
        }
    
        // Loop through the hay stack
        // Loop through the given word
        for(int i = 0; i < haystack.length(); i++) {
            for(int j = 0; j < word.length(); j++) {
                if(haystack.charAt(i + j) != word.charAt(index))
            }
        }
